...
def run_script():
    ...
    if len(script) > 0 and len(save) > 0:
        command = "python scripts/" + script + " --input " + save
...
script_listbox = tk.Listbox(master=window, height=LIST_HEIGHT, listvariable=tk.StringVar(value=scripts), font=font)
...
save_listbox = tk.Listbox(master=window, height=LIST_HEIGHT, listvariable=tk.StringVar(value=saves), font=font)
...
chosen_script_entry = tk.Entry(master=window, width=45, font=font)
...
chosen_save_label.grid(column=2, row=14, padx=10, pady=0)
chosen_save_entry = tk.Entry(master=window, width=45, font=font)
chosen_save_entry.grid(column=2, row=15, padx=10, pady=0)
chosen_save_entry.configure(state='disabled')

run_button = tk.Button(master=window, text="Run script", command=run_script, font=font)
run_button.grid(column=2, row=17, padx=10, pady=0)
run_label_var = tk.StringVar()
run_label = tk.Label(master=window, textvariable=run_label_var, width=45, font=font_small)
run_label.grid(column=2, row=18, padx=10, pady=0)
